(() => {
  ...
  function createNextButton() {
    if (document.querySelectorAll('.prevNextBut').length>0&&document.querySelectorAll('.prevNextBut')[document.querySelectorAll('.prevNextBut').length-1].innerText=='Sonraki') {
      if (document.querySelector('.arc-dgr-next')) {
        
        document.querySelector('.arc-dgr-next').remove()
      }
        document.querySelector('#searchResultsSearchForm').insertAdjacentHTML('afterbegin',`<div class="arc-dgr-next">Sonraki</div>`);

        document.querySelector('.arc-dgr-next').addEventListener('click',()=>{
          document.querySelectorAll('.prevNextBut')[document.querySelectorAll('.prevNextBut').length-1].click()
        })
  ...
  function injectEvaluateButton() {
    ...
    const apiKeyButton = createApiKeyButton();
    ...
    if (targetElement) {
      ...
      apiKeyButton.addEventListener('click', openApiKeyPopup);
    }
  }
  ...
  function createApiKeyButton() {
    ...
    button.id = 'apiKeyButton';
    button.textContent = 'API Keyini Gir';
    return button;
  }
  ...
  function openApiKeyPopup() {
    ...
    const existingApiKey = window.atob(localStorage.getItem('openaiApiKey'));
    if (existingApiKey) {
    ...
    saveButton.addEventListener('click', () => {
      ...
      if (apiKey) {
        ...
        showNotification('API anahtarınız kaydedildi.');
        document.body.removeChild(overlay);
      } else {
        showNotification('Lütfen geçerli bir API anahtarı girin.');
      }
    });
...
function injectStyles() {
  ...
  const styles = `
    ...
    /* Popup stilleri */
    #apiKeyOverlay {
      /* Overlay stilleri zaten inline olarak ayarlandı */
    }
    #apiKeyOverlay input {
      padding: 8px;
      font-size: 16px;
  ...
  async function evaluateCar() {
    ...
    try {
      ...
      const userApiKey = window.atob(localStorage.getItem('openaiApiKey'));
      if (userApiKey) {
        carData.openaiApiKey = userApiKey;
      }
      ...
      // Butonu tekrar etkinleştir ve orijinal metni geri getir
      evaluateButton.disabled = false;
      evaluateButton.textContent = originalButtonText;
    }
  }
...
function displayEvaluationResult(data, carData) {
 ...
 // Kalan kullanım hakkını ekleyin
